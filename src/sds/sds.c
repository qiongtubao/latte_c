/*
 * sds.c - 动态字符串 (SDS) 实现文件
 * 
 * Latte C 库核心组件：高效安全的字符串管理实现
 * 
 * 主要功能：
 * 1. 智能类型选择 (hdr5/8/16/32/64) 优化内存使用
 * 2. 内存预分配和惰性释放策略
 * 3. 安全字符串操作函数集
 * 4. 二进制安全支持
 * 
...
/*
 * sdsHdrSize - 获取指定类型 SDS 头的大小
 * 参数：type - SDS 类型标识 (SDS_TYPE_5/8/16/32/64)
 * 返回：头结构占用的字节数
 * 作用：根据类型计算头大小，用于指针偏移计算
 */
...
/*
 * sds_remove_free_space - 释放 SDS 未使用的内存
 */
