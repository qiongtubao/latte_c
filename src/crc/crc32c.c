...
uint32_t crc32c_extend(uint32_t crc, const char* data, int len) {
    ...
    if (accelerate == NULL) {
        accelerate = canAccelerateCRC32C();
    }
    if (accelerate) {
        return acceleratedCRC32C(crc, data, len);
    }
...
/**
 * 
 * 掩码（mask）主要用于两个主要目的：数据预处理和后处理。

      数据预处理（Data Preprocessing）
      在计算CRC校验值之前，发送方可能需要对要传输的数据进行预处理，以避免在数据中嵌入CRC校验值本身。这是因为CRC校验值作为数据的一部分被传输时，如果数据中恰好包含相同的CRC值，这将导致校验失败，因为CRC算法会错误地认为这部分数据也是校验值的一部分。

      为了克服这个问题，发送方会对数据进行预处理，通过XOR操作与一个掩码进行异或运算。这样做的目的是改变数据中可能存在的CRC值，使其在传输过程中不会与真正的CRC值混淆。接收方收到数据后，会使用相同的掩码进行反向操作，恢复原始数据，然后再计算CRC值进行校验。
