...
g_counter_meta* create_g_counter_meta(long long gid, long long vcu) {
    g_counter_meta* meta = zmalloc(sizeof(g_counter_meta));
    meta->gid = gid;
    meta->vcu = vcu;
...
int g_counter_meta_to_str(g_counter_meta* meta, char* buf) {
    ...
    if(meta->data_type !=VALUE_TYPE_SDS) {
        len += len += copy_sds_to_buf(buf + len, meta->v);
    } else {
        len += ll2string(buf + len, 21, sdslen(meta->value.s));
        buf[len++] = ':';
        len += copy_sds_to_buf(buf + len, meta->value.s);
    }
    return len;
}
...
int str_2_value_and_g_counter_metas(sds info, long long*type, union all_type* value,  g_counter_meta** g) {
    ...
    if(*type != VALUE_TYPE_SDS) { sdsfree(value); }
